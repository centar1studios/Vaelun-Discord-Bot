...
"""
        ...
        await interaction.response.send_message(embed=persona_embed(persona, "Server Persona", description))
    ...
    async def name(self, interaction: discord.Interaction, name: str):
        ...
        await interaction.response.send_message(embed=success_embed("Persona name updated."), ephemeral=True)

    @app_commands.command(name="bio", description="Set Persona bio.")
    @app_commands.checks.has_permissions(manage_guild=True)
    async def bio(self, interaction: discord.Interaction, bio: str):
        if len(bio) > 500:
            await interaction.response.send_message(embed=error_embed("Bio must be 500 characters or less."), ephemeral=True)
            return

        self.bot.db.update_persona(interaction.guild.id, "bio", bio)
        await interaction.response.send_message(embed=success_embed("Persona bio updated."), ephemeral=True)
    ...
    async def avatar(self, interaction: discord.Interaction, avatar_url: str):
        if not avatar_url.startswith("http://") and not avatar_url.startswith("https://"):
            await interaction.response.send_message(embed=error_embed("Use a valid image URL."), ephemeral=True)
            return

        self.bot.db.update_persona(interaction.guild.id, "avatar_url", avatar_url)
        await interaction.response.send_message(embed=success_embed("Persona avatar updated."), ephemeral=True)
    ...
    async def color(self, interaction: discord.Interaction, color: str):
        if not HEX_RE.match(color):
            await interaction.response.send_message(embed=error_embed("Use a valid hex color like #9B7BFF."), ephemeral=True)
            return
        ...
        await interaction.response.send_message(embed=success_embed("Persona color updated."), ephemeral=True)
    ...
    async def footer(self, interaction: discord.Interaction, footer: str):
        ...
        await interaction.response.send_message(embed=success_embed("Persona footer updated."), ephemeral=True)
    ...
    async def nickname(self, interaction: discord.Interaction, nickname: str):
        try:
            ...
            await interaction.response.send_message(embed=success_embed("Bot nickname updated."), ephemeral=True)
        except discord.Forbidden:
            await interaction.response.send_message(embed=error_embed("I need permission to change my nickname."), ephemeral=True)
    ...
    async def reset(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(embed=success_embed("Persona reset."), ephemeral=True)
